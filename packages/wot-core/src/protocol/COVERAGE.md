...
| `wot-sync@0.1` | focused protocol tests | Sync 003 broker device registration disposition | `sync/broker-device-registration-disposition.ts` | Full for slice | Pure post-authentication classification for active reconnects, first registration actions, global `deviceId` conflict, revoked-device rejection, and revocation-wins races. DID/deviceId syntax and malformed register-envelope behavior remain out of scope and tracked in `real-life-org/wot-spec#28`; inactive/deactivated retention semantics remain out of scope and tracked in `real-life-org/wot-spec#27`; revoked foreign `deviceId` reuse semantics are conservatively treated as conflict tombstones pending `real-life-org/wot-spec#29`. |
