...
| `wot-sync@0.1` | `profile-service-response.schema.json`, Sync 004 prose, and focused protocol tests | Sync 004 `/p/{did}` profile-service profile resource | `sync/profile-service-resource.ts` | Partial | Covers pure payload validation, DID/path and DID-document consistency, profile metadata key-material rejection, deterministic server PUT version decisions, client rollback detection, and generic Identity 002 compact EdDSA JWS verification over the exact received signing input. Profile-service resource-specific JWS `typ`, `/p/{did}/v` and `/p/{did}/a` list-resource schemas, additional-properties ownership, and vector ownership remain deferred to `real-life-org/wot-spec#34`. |
...
- Sync 004 profile-service resource-specific JWS `typ`, `/p/{did}/v` and `/p/{did}/a` list-resource schemas, and vector ownership are deferred pending `real-life-org/wot-spec#34`.
