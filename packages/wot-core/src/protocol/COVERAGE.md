...
| `wot-sync@0.1` | focused protocol tests | Sync 003 broker per-device inbox routing and ACK scoping | `sync/broker-inbox-disposition.ts` | Full for slice | `BrokerInboxDisposition.test.ts` covers pure per-active-device target computation, revoked/deactivated exclusion with cleanup guidance, self-addressed sender exclusion, authenticated per-device ACK scoping, and full-delivery calculation. Inactive-device, TTL, and long-offline GC policy remain `[NEEDS CLARIFICATION: wot-spec #27]` and are not implemented by this helper. |
