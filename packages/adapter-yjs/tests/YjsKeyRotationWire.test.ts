...
// Inbox-Wire-Form (Sync 003 Z.343-470): die 3 Membership-Typen reisen als
// DIDComm-Plaintext-Envelope, Body = ECIES-Container {epk, nonce, ciphertext},
// Klartext-Body im Inner-JWS-Payload. C6: Key-Material nie im Klartext auf dem
// Wire. C5/S2: removeMember rotiert nur an Remaining, member-updated alle.
// ACK-Ownership (K1): ack/1.0 kommt vom Reception-Host nach Apply/durabler
// Pufferung — fehlgeschlagene Verarbeitung sendet KEIN ack.
...
describe('Yjs inbox wire form (C5/C6/S2 + Inner-JWS + ack/1.0)', () => {
  ...
  it('C6: space-invite ist ein encrypted DIDComm-Envelope ohne Klartext-Key-Material; Body dekodiert + validiert', async () => {
    ...
    // K4 encrypted-Outer-Form: ECIES-Container + Extension-Feld, kein Klartext-Body.
  ...
  it('S4: applies a spec-conformant invite WITHOUT encryptedDocSnapshot (keys persisted, space registered)', async () => {
    ...
    // Pure spec invite via Workflow — NO encryptedDocSnapshot extension, doc arrives via sync.
