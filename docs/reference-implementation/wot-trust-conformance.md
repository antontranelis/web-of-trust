...
- **Open questions:** `wot-spec#47` tracks the exact Trust 002 `jti` nonce matching grammar for active challenge acceptance and nonce-history replay classification. This slice extracts UUID-shaped nonces from `jti` for replay protection while the protocol helper continues to own active challenge acceptance.
